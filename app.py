...
# Choose data source
data_source = st.sidebar.radio("📥 Data Source", ["Gmail API", "Sample CSV"])
...
for idx, row in filtered_df.iterrows():
    with st.expander(f"{row['subject']} | {row['sender']} | {row['priority']} | {row['sentiment']}"):
        ...
        if st.button(f"✉️ Send Reply to {row['sender']}", key=f"send_{idx}"):
            if data_source == "Gmail API":
                try:
                    send_gmail(service, row['sender'], f"Re: {row['subject']}", draft)
                    st.success(f"✅ Reply sent to {row['sender']}")
                except Exception as e:
